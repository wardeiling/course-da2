...
tokens = word_tokenizer(tolower(hp_tokens2$word))
it = itoken(tokens)
...
```{r}
hp_wv_context <- glove$components$context
hp_wv_main_context <- hp_wv_main + hp_wv_context
```
...
sim2(hp_wv_main_context, c("harry", "death", "love"), n = 10, method = "cosine")
```
...
hp_wv_main_context["harry",] + hp_wv_main_context["love",] - hp_wv_main_context["death",]
```
