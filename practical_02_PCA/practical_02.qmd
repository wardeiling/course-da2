...
```{r}
Dat <- read.table("practical_02_PCA/data/Example1.dat")
Mat <- as.matrix(Dat)
...
```{r}
biplot(mtcars.pca, choices = c(1,2), cex = 0.5)
```
...
```{r}
biplot(mtcars.pca, choices = c(1,3), cex = 0.5)
```
...
According to the screeplot, we should extract 2 principal components, as if we draw a straight line it passes through PC 3 to 9, and if we then take the elements before the elbow, only PC 1 and PC2 remain.
