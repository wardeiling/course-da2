...
# library(reticulate)
# py_config()
# py_run_string("import requests; print(requests.get('https://storage.googleapis.com/tensorflow/tf-keras-datasets/mnist.npz').status_code)")
# py_run_string("import ssl; print(ssl.OPENSSL_VERSION)")
...
```{r}
mnist <- keras3::dataset_mnist(path = "practical_06_deep-learning/mnist.npz")
```
...
```{r}
deep_nn <- keras_model_sequential(input_shape = c(28, 28)) %>% 
  layer_flatten() %>% 
...
```{r}
deep_nn %>% fit(x = mnist$train$x, y = mnist$train$y, epochs = 10, validation_split = 0.2, verbose = 1)
```
...
improved_cnn %>% fit(x = mnist$train$x, y = mnist$train$y, epochs = 10, validation_split = 0.2, verbose = 1)
```
